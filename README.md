...
查询洛奇游戏魔法释放卷轴道具数据，数据来自 prilus 资源服务器。
...
查询洛奇装备改造方案，含消耗、属性修改、幸运改造概率、宝石改造，数据来自 prilus 资源服务器。

| 操作 | 触发方式 |
|------|---------|
| 搜索 | `up <关键词>`，如 `up 释魂 弓` |
| 通配符搜索 | `up 释魂*弓`，用 `*` 作通配符 |
| 精准查询 | `up <id>`，如 `up 1040054` |
...
| 操作 | 触发方式 |
|------|---------|
| 搜索 | `totem <关键词>`，如 `totem 学会` |
| 通配符搜索 | `totem *纪念币`，用 `*` 作通配符 |
| 精准查询 | `totem <id>`，如 `totem 52358` |

### 农场物查询（Miniature）

查询洛奇游戏农场物信息，含属性加成、类型分类、图片，数据来自 prilus 资源服务器。

| 操作 | 触发方式 |
|------|---------|
| 搜索 | `mini <关键词>`，如 `mini 英雄` |
| 精准查询 | `mini <id>`，如 `mini 155` |
...
│   ├── qa/                       # 问答知识库插件
│   ├── video/                    # 视频下载插件
│   ├── optionset/                # 魔法释放卷轴查询插件
│   ├── production/               # 装备制作配方查询插件
